...
def get_flags():
    ...
    parser = ap.ArgumentParser(prog='abqaq.py',  description="Albuquerque Air Quality Parser", epilog="Parses file.dat or reads from stdin and converts to YAML(o JSON if '-j' flag given, and writes to stdout")
    ...
    parser.add_argument('-c', '--config', action='store_true', help=f"Write config file to {CONFIG_FILE} and exit")
