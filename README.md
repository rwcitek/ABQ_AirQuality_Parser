...
If the current directory contains a file : '.abqaq.yml', then that file is read
and processed before any other action takes place. This file is in the YAML
format and can be editted by any any text editor. It controls the following options:
...
The configuration file: './.abqaq.yml' can be generated by the '--check' option.

```bash
# Say we want to always suppress info messages and and write JSON output:
python abqaq.py  --quiet --json --check
